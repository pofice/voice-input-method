...
class MyWindow(QWidget):
    ...
    def __init__(self):
        ...
        # 初始化模型
        model_dir = "/home/pofice/Desktop/voice-input-method/speech_seaco_paraformer_large_asr_nat-zh-cn-16k-common-vocab8404-pytorch/"
        self.model = SeacoParaformer(model_dir, batch_size=1, quantize=True)
        self.model.language = "zh"  # 或根据模型实际语言设置
    ...
    def transcribe_audio_thread(self):
        ...
        if result and 'preds' in result[0]:
            ...
            # preds is a tuple: (str, list), we want the string part
            if isinstance(preds, tuple):
                text = preds[0]
            else:
                text = preds
            formatted_transcription = re.sub(r'(?<=[\u4e00-\u9fff]) (?=[\u4e00-\u9fff])', '', text)
